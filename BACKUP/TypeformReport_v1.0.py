...
df_BDDF = df[df.PRODUÇÃO == 'BDDF'].reset_index()

df_TBDDF = df_BDDF['DIA'].value_counts().reset_index()      #soma as ocorrencias de um valor
df_TBDDF.columns = ['Dia', 'Total de artes']
df_TBDDF_final = df_TBDDF.sort_values(by=['Dia'])

plt.subplot(4,1,4)
sns.set_style('darkgrid')
sns.barplot(data=df_TBDDF_final, x='Dia', y='Total de artes').set_title('Total Artes BDDF no Mês')
