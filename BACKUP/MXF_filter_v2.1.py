...
##
##FUNÇÕES:
##    -salvar 3 arquivos TypeForm_YY-MM-DD.csv para os últimos 3 dias
...
CSV_dir = "S:\\A R Q U I V O S\\FERRAMENTAS\\AutoTypeform\\TEMP\\CSV_FINAL"
var_nula = 'x'
...
# FUNÇÃO FILTRO - insere um valor NULO à uma posição vazia da tabela
...
def Fn_Create_Day_DB(NumDaysBefore):
    ...
    saveMonthPath = str(CSV_dir) + '\\relatorios_' + str(saveMonth)
    if not os.path.isdir(saveMonthPath) == True:
        os.makedirs(saveMonthPath)  

    CSV_filter = r"S:\\A R Q U I V O S\\FERRAMENTAS\\AutoTypeform\\TEMP\\CSV_FINAL\\relatorios_" + str(saveMonth)
    path = os.chdir(r"S:\\A R Q U I V O S\\FERRAMENTAS\\AutoTypeform\\TEMP\\CSV_FINAL\\relatorios_" + str(saveMonth))
    ...
    savePath = str(CSV_dir) + '\\relatorios_' + str(saveMonth) + '\\TypeForm_' + str(saveDate) + '.csv'
